...
{  
    ...
    { 
        ...
        //                                     where 
        //                                     complainant_id_fk = ? 
        //                                     and district_council_id_fk = ? 
        //                                     and complaint_category_id_fk = ? 
        //                                     and complaint_detail = ?',
        //                                     array($data_arr['complainant_id_fk'],$data_arr['district_council_id_fk'],$data_arr['complaint_category_id_fk'],$data_arr['complaint_detail']))->result_array();
       $find_complaint=    $this->db->select('*')
                                    ->from('complaints')
                                    ->where('complainant_id_fk',$data_arr['complainant_id_fk'])
                                    ->where('district_council_id_fk',$data_arr['district_council_id_fk'])
    ...
    public function complaints_get($data_arr)
    {
        ...
        array_push($join_arr,array('j_table'=>'district_councils','t_table'=>'complaints','select_columns'=>array('district_council_name'=>0),'t_column'=>'district_council_id_fk','j_column'=>'district_council_id'));
        ...
        $optional_cond_arr = array('complaint_id','complainant_id_fk','registered_by_user','district_council_id_fk','complaint_category_id_fk','complaint_status_id_fk');
    ...
    {
        $required_fields = array('complaint_id_fk'=>0,'user_id_fk'=>0,'complaint_status_id_fk'=>0,'respondent_id_fk'=>0,'respondent_district_id_fk'=>0,'complaint_remarks_detail'=>0,'complaint_remarks_timestamp'=>0,'complaint_remarks_status'=>0);
        $missing = array_diff_key($required_fields,$data_arr);
        ...
        $find_complaint_remarks = $this->db->query('select * from complaint_remarks where complaint_id_fk = ? complaint_remarks_detail = ?',array($data_arr['complaint_id_fk'],$data_arr['complaint_remarks_detail']))->result_array();
        ...
        else
        {
            ...
            if($insert_remarks != false)
            {
                ...
                return array('response'=>1,'response_msg'=>'Remarks Submitted Successfully','remarks_id'=>$remarks_id);
            }
            else
            {
    ...
    { 
      ...
      $this->db->join('district_councils', 'district_councils.district_council_id = complaints.district_council_id_fk','left');
    ...
    function get_complaint_remarks($complaint_id)
    {
        ...
        $this->db->join('districts', 'districts.district_id=complaints.district_council_id_fk','left');
